...
const missingAssets = audit.rows.filter((row) => !fs.existsSync(`playnice-site/public${row.assetPath}`));
...
console.log(`PASS  all ${audit.uniqueNotes} referenced notes have matching playnice-site/public/note-map WebP assets`);
