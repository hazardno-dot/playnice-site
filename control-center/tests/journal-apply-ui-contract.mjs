...
assert.match(source, /reviewed_at/, "UI must require preparation after the latest approval");
...
assert.match(source, /Prepare refresh/, "existing PR must expose a new preparation step after re-approval");
assert.match(source, /callApply\("refresh"\)/, "existing prepared PR must call explicit refresh action");
assert.match(source, /Refresh draft PR/, "existing prepared PR must expose refresh control");
