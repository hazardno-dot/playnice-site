-- PlayNice Control Center / Hero v1 schema draft
-- IMPORTANT: repository artifact only. This file is NOT applied to Production.
-- Phase 1 keeps the Control Center Hero module read-only and backed by heroSnapshot.mjs.
-- When a dedicated Supabase development branch is approved, apply and verify there first.
...
create table if not exists public.hero_slides (
  ...
  constraint hero_product_action_contract check (
    action_type <> 'product' or product_slug is not null
  ),
  constraint hero_collection_action_contract check (
    action_type <> 'collection' or cardinality(collection_slugs) > 0
  ),
  constraint hero_manifesto_action_contract check (
    action_type <> 'manifesto' or manifesto_type is not null
  )
);
...
-- Intentionally no anon/authenticated grants or policies in v1.
-- The access policy must be added only after we confirm the Control Center
-- authorization model on a Supabase development branch.
