...
import { ANNOUNCEMENT_ACTIONS, ANNOUNCEMENT_TONES, auditAnnouncementDraft, normalizeAnnouncementDraft } from "./announcementDraft.mjs";
...
export default function AnnouncementManager() {
  ...
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      const { data, error: loadError } = await supabase.from("announcement_drafts").select("announcement_key,payload,review_status,updated_at").order("updated_at", { ascending: false });
      if (cancelled) return;
  ...
  const saveDraft = async (payload) => {
    ...
    try {
      ...
      const { data, error: saveError } = await supabase.from("announcement_drafts").upsert({ announcement_key: validation.payload.id, payload: validation.payload, created_by: authData.user.id }, { onConflict: "announcement_key" }).select("announcement_key,payload,review_status,updated_at").single();
  ...
  return createPortal(<section className="announcement-manager">
    <div className="announcement-audit-strip"><div><span>EDITORIAL ANNOUNCEMENTS</span><strong>{enabledCount} enabled · {rows.length} total · {Object.keys(draftRows).length} drafts</strong></div><div className="announcement-audit-note">DRAFT MODE · storefront generated config unchanged</div></div>
    ...
    <div className="announcement-manager-grid">
      <div className="announcement-list-panel">
        ...
        <div className="announcement-list">{rows.map((item) => <article className="announcement-row" key={item.id}>
          <div className="announcement-row-top"><div><span className={`announcement-status ${item.enabled ? "enabled" : "disabled"}`}>{item.enabled ? "ENABLED" : "DISABLED"}</span>{item.__draft ? <span className="announcement-draft-badge">DRAFT</span> : null}<code>{item.id}</code></div><strong>Priority {item.priority ?? 0}</strong></div>
          ...
          <div className="announcement-row-actions"><button onClick={() => startEdit(item)}>{item.__draft ? "Edit draft" : "Create draft"}</button>{item.__draft ? <button className="danger" onClick={() => discardDraft(item.id)}>Discard draft</button> : null}</div>
        </article>)}{!rows.length ? <div className="announcement-empty">No editorial announcements in generated config.</div> : null}</div>
      </div>
      <aside className="announcement-preview-panel"><div className="announcement-section-head"><div><span>WORKING COPY PREVIEW</span><h2>Storefront text</h2></div></div><div className="announcement-preview-group"><span>SR</span>{rows.filter((item) => item.enabled).map((item) => <AnnouncementPreview key={`sr-${item.id}`} item={item} lang="sr" />)}</div><div className="announcement-preview-group"><span>EN</span>{rows.filter((item) => item.enabled).map((item) => <AnnouncementPreview key={`en-${item.id}`} item={item} lang="en" />)}</div><div className="announcement-scope-note"><strong>V1 SCOPE</strong><p>Drafts change only editorial/promo items. Automatic New Products, Latest Journal, Forever sponsored content and cart/shipping system messages stay outside this module.</p></div></aside>
    </div>
  </section>, slot);
}
