...
function LangPair({label,value={},onChange,multiline=false,hint=""}){const C=multiline?TextField:Field;return <div className="lang-pair"><C label={`${label} · SR`} value={value?.sr||""} onChange={(v)=>onChange({...value,sr:v})}/><C label={`${label} · EN`} value={value?.en||""} onChange={(v)=>onChange({...value,en:v})}/>{hint?<small className="field-hint">{hint}</small>:null}</div>}
