...
function parseArrayLiteral(raw, label) {
  try {
    const parsed = vm.runInNewContext(`(${raw})`, Object.create(null), { timeout: 1000 });
    if (!Array.isArray(parsed)) throw new Error("not an array");
    return JSON.parse(JSON.stringify(parsed));
  } catch (error) {
    throw new Error(`Could not safely parse ${label}: ${error.message}`);
  }
}
...
function extractHardcodedHero(source) {
  ...
  return { start, end: shuffleStart, raw, parsed: parseArrayLiteral(raw, "BASE_HERO_SLIDES") };
}
...
function addConfigImportAndRemoveBlock(appSource, hardcoded) {
  if (!hardcoded) return appSource;
  ...
  const refreshed = extractHardcodedHero(next);
  if (!refreshed) throw new Error("Could not locate hardcoded Hero block after import insertion.");
  next = next.slice(0, refreshed.start) + next.slice(refreshed.end);
  return next;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const slideRows = await slidesResponse.json();
    ...
    if (hardcoded) {
      const nextApp = addConfigImportAndRemoveBlock(appSource, hardcoded);
    ...
    if (!patchResponse.ok) throw new Error("Preview PR was created, but Hero draft metadata could not be updated.");

    return json(res, 200, { ok: true, branch, pr_number: pr.number, pr_url: pr.html_url, files: changedFiles, base_sha: baseSha });
  } catch (error) {
